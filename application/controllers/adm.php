...
class Adm extends CI_Controller
{
	...
	public function m_siswa()
	{
		...
		if ($uri3 == "det") {
			...
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE m_siswa SET nama = '" . bersih($p, "nama") . "', nis = '" . bersih($p, "nis") . "', kelas = '" . bersih($p, "kelas") . "', email = '" . bersih($p, "email") . "'	WHERE id = '" . bersih($p, "id") . "'");
				$ket = "edit";
			} else {
				$ket = "tambah";
				// var_dump(bersih($p, "email"));
				// die;
			...
			$det_user = $this->db->query("SELECT id, nis, email, nama FROM m_siswa WHERE id = '$uri4'")->row();
			$admin = $this->db->query("SELECT email, pass FROM m_admin WHERE id = 1")->row();

			// echo $admin->email;
			// echo '  ' .$admin->pass;
			// exit();
			$data = array(
				'username' => $det_user->nis,
				'password' => md5('123'),
				'level' => 'siswa',
				'kon_id' => $det_user->id
			);
			...
			if (!empty($det_user)) {
				...
				// send email CI library
				$config = [
					'protocol'  => 'smtp',
					'smtp_host' => 'ssl://smtp.googlemail.com',
					'smtp_user' => $admin->email,
					'smtp_pass' => $admin->pass,
					'smtp_port' => 465,
					'mailtype'  => 'html',
					'charset'   => 'utf-8',
					'newline'   => "\r\n"
			...
			exit();
		} else if ($uri3 == "ambil_matpel") {

			$matpel = $this->db->query("SELECT m_mapel.*,
			...
			$this->j($ret_arr);
			exit;
		} else if ($uri3 == "simpan_matpel") {
			$ket 	= "";
			//echo var_dump($p);

			$ambil_matpel = $this->db->query("SELECT id FROM m_mapel ORDER BY id ASC")->result();
			if (!empty($ambil_matpel)) {
				foreach ($ambil_matpel as $a) {
	...
	public function m_guru()
	{
		...
		if ($uri3 == "det") {
			...
			if ($p->id != 0) {
				...
				$this->db->update('m_guru', $data, array('id' => bersih($p, "id")));
			...
			$det_user = $this->db->query("SELECT * FROM m_guru WHERE id = '$uri4'")->row();

			// $username = str_replace(" ", "_", $det_user->nama);
			// echo $username;
			// exit();

			if (!empty($det_user)) {
				$data = array(
					'username' => $det_user->nip,
					'password' => md5('123'),
					'level' => 'guru',
					'kon_id' => $det_user->id,
				);
				...
				// die;
				$this->db->insert('m_admin', $data);
				// $this->db->query("INSERT INTO m_admin VALUES (null, 'guru" . $det_user->id . "', md5('123'), 'guru', '" . $det_user->id . "')");
				$ret_arr['status'] 	= "ok";
				$ret_arr['caption']	= "tambah user sukses";
				$this->j($ret_arr);
			} else {
	...
	public function m_mapel()
	{
		...
		$a['data'] = $this->db->query("SELECT m_mapel.* FROM m_mapel ORDER BY nama ASC ")->result();
		...
		if ($uri3 == "det") {
			...
			if ($p->id != 0) {
				...
				// WHERE id = '" . bersih($p, "id") . "'");
				$data = [
					'nama' => bersih($p, "nama") . ' ' . bersih($p, "kelas")
				];
				// var_dump($data);
				// die;
				$this->db->update('m_mapel', $data, ['id' =>  bersih($p, "id")]);
				$ket = "edit";
			} else {
				$ket = "tambah";
	...
	public function m_materi()
	{
		...
		if ($a['sess_level'] == "guru") {
			$a['p_mapel'] = obj_to_array($this->db->query("SELECT * FROM m_mapel WHERE id IN (SELECT id_mapel FROM tr_guru_mapel WHERE id_guru = '" . $a['sess_konid'] . "')")->result(), "id,nama");
			$a['tambah'] = '<a class="btn btn-success btn-sm" onclick="return m_soal_e(0);"><i class="glyphicon glyphicon-plus" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Tambah Data</a>';
			$a['admin'] = '';
			$a['siswa'] = 'hidden';
			$a['hapus_file'] = '';
		} else if ($a['sess_level'] == "admin") {
		...
		if ($uri3 == "det") {
			if ($uri4 != "0") {
				$a = $this->db->query("SELECT * FROM m_materi WHERE id = '$uri4'")->row_array();
			} else {
				$a = array("id" => "", "id_guru" => $a['sess_konid'], "id_mapel" => "", "file" => "", "nama" => "");
			}

			$this->j($a);
			exit();
		} else if ($uri3 == "hapus_gambar") {
			$nama_file = $this->db->query("SELECT file FROM m_materi WHERE id = '" . $uri5 . "'")->row();
			$this->db->query("UPDATE m_materi SET file = '' WHERE id = '" . $uri5 . "'");
			@unlink("./upload/materi/" . $nama_file->file);
			redirect('adm/m_materi/pilih_mapel/' . $uri4);
		} else if ($uri3 == "pilih_mapel") {
			if ($a['sess_level'] == "guru") {
				$a['data'] = $this->db->query("SELECT m_materi.*, m_guru.nama AS nama_guru FROM m_materi INNER JOIN m_guru ON m_materi.id_guru = m_guru.id WHERE m_materi.id_guru = '" . $a['sess_konid'] . "' AND m_materi.id_mapel = '$uri4'")->result();
			} else {
				$a['data'] = $this->db->query("SELECT m_materi.*, m_guru.nama AS nama_guru FROM m_materi INNER JOIN m_guru ON m_materi.id_guru = m_guru.id WHERE m_materi.id_mapel = '$uri4'")->result();
			}
			//echo $this->db->last_query();
			$a['p']	= "m_materi";
		} else if ($uri3 == "simpan") {
			$status = "";
			$msg = "";
			$file_element_name = 'file_materi';

			//echo var_dump($_POST);

			$pembuat_materi = ($a['sess_level'] == "admin") ? $_POST['id_guru'] : $a['sess_konid'];
			$pembuat_materi_u = ($a['sess_level'] == "admin") ? ", id_guru = '" . $_POST['id_guru'] . "'" : "";
			...
			if ($status != "error") {
				...
				if (!$this->upload->do_upload($file_element_name)) {
					...
					if ($_POST['id'] != "") {
						...
						$this->db->update('m_materi', $form, array('id' => $_POST['id']));
					} else {
						$form = array(
							'id_guru' => $_POST['id_guru'],
							'id_mapel' => $_POST['id_mapel'],
					...
					$ambil_file = $this->db->query("SELECT file FROM m_materi WHERE id = '" . $_POST['id'] . "'")->row();
					if ($_POST['id'] != "") {
						$form = array(
							'id_guru' => $_POST['id_guru'],
							'id_mapel' => $_POST['id_mapel'],
					...
					@unlink("./upload/materi/" . $ambil_file->file);
				}
				//@unlink($_FILES[$file_element_name]);
			}
			$jeson['status'] = $status;
			$jeson['id_mapel'] = $_POST['id_mapel'];
			$jeson['msg'] = "Materi berhasil disimpan. " . $msg;
			header('Content-Type: text/html');
			echo json_encode($jeson);
			exit;
		} else if ($uri3 == "hapus") {
			$nama_file = $this->db->query("SELECT file FROM m_materi WHERE id = '" . $uri4 . "'")->row();
			$this->db->query("DELETE FROM m_materi WHERE id = '" . $uri4 . "'");
			@unlink("./upload/materi/" . $nama_file->file);
			$ret_arr['status'] 	= "ok";
			$ret_arr['caption']	= "hapus sukses";
			$this->j($ret_arr);
			exit();
		} else if ($uri3 == "download") {
			$nama_file = $this->db->query("SELECT id_mapel, file FROM m_materi WHERE id = '" . $uri4 . "'")->row();
			$loc_file =  base_url() . 'upload/materi/' . $nama_file->file;

			header('Content-Description: File Transfer');
			header('Content-Type: application/octet-stream');
			header('Content-Disposition: attachment; filename=' . basename($loc_file));
			header('Content-Transfer-Encoding: binary');
			header('Expires: 0');
			header('Cache-Control: private');
			header('Pragma: private');
			header('Content-Length: ' . filesize($loc_file));
			ob_clean();
			flush();
			readfile($loc_file);

			$balik = base_url() . 'adm/m_materi/pilih_mapel/' . $nama_file->id_mapel;
			redirect($balik);
			exit();
		} else {
			$a['p']	= "m_materi";
	...
	public function email()
	{
		...
		if ($uri3 == "det") {
			$a = $this->db->query("SELECT * FROM m_admin WHERE id = 1")->row();
			$this->j($a);
			exit();
		} else if ($uri3 == "simpan") {
			$data = array(
			...
			// var_dump($data);
			// die;
			$this->db->update('m_admin', $data, array('id' => 1));
			redirect(base_url() . 'adm/email');
			// $this->db->query("UPDATE m_admin SET nama = '" . bersih($p, "nama") . "', nim = '" . bersih($p, "nim") . "', jurusan = '" . bersih($p, "jurusan") . "', email = '" . bersih($p, "email") . "'	WHERE id = '" . bersih($p, "id") . "'");
			// $ket = "edit";
