...
function m_soal_h(id) {
	if (confirm('Apakah anda yakin ingin menghapus data soal ini ?')) {
		$.ajax({
			type: "GET",
...
// Materi
function m_materi_e(id) {
	$("#m_soal").modal('show');
	$.ajax({
		type: "GET",
		url: base_url + "adm/m_materi/det/" + id,
		success: function (data) {
			$("#id").val(data.id);
			//$("#id_mapel").val(data.id_mapel);
			$("#id_guru").val(data.id_guru);
...
function m_materi_h(id) {
	if (confirm('Apakah anda yakin ingin menghapus data soal ini ?')) {
		$.ajax({
			type: "GET",
			url: base_url + "adm/m_materi/hapus/" + id,
			success: function (response) {
				if (response.status == "ok") {
					window.location.assign(base_url + "adm/m_materi");
				} else {
					console.log('gagal');
				}
			}
...
function m_materi_d(id) {
	$.ajax({
		type: "GET",
		url: base_url + "adm/m_materi/download/" + id,
		success: function (response) {
			if (response.status == "ok") {
				// window.location.assign(base_url + "adm/m_materi/pilih_mapel/" + response.mapel);
...
function m_ujian_h(id) {
	if (confirm('Apakah anda yakin akan menghapus data ujian ini ?')) {
		$.ajax({
			type: "GET",
...
function m_siswa_u(id) {
	if (confirm('Data ini akan diaktifkan. Username otomatis adalah "(nis siswa)", dan Password default adalah "123" silahkan dirubah pada menu "Ubah Password" apabila sudah login. Lanjutkan ?')) {
		$.ajax({
			type: "GET",
...
function m_siswa_matpel(id) {
	$.ajax({
		type: "GET",
		url: base_url + "adm/m_siswa/ambil_matpel/" + id,
		success: function (data) {
			if (data.status == "ok") {
				var jml_data = Object.keys(data.data).length;
				var hate = '<div class="modal fade" id="m_siswa_matpel" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 id="myModalLabel">Setting Mata Pelajaran</h4></div><div class="modal-body"><form name="f_siswa_matpel" id="f_siswa_matpel" method="post" onsubmit="return m_siswa_matpel_s();"><input type="hidden" name="id_mhs" id="id_mhs" value="' + id + '"><div id="konfirmasi"></div>';
...
function m_siswa_matpel_s() {
	var f_asal = $("#f_siswa_matpel");
	var form = getFormData(f_asal);

	$.ajax({
		type: "POST",
		url: base_url + "adm/m_siswa/simpan_matpel",
		data: JSON.stringify(form),
		dataType: 'json',
		contentType: 'application/json; charset=utf-8'
	}).done(function (response) {
...
function m_guru_u(id) {
	if (confirm('Data ini akan diaktifkan. Username otomatis adalah "(nip guru)", dan Password default adalah "123" silahkan dirubah pada menu "Ubah Password" apabila sudah login. Lanjutkan ?')) {
		$.ajax({
			type: "GET",
...
function m_guru_matpel(id) {
	$.ajax({
		type: "GET",
		url: base_url + "adm/m_guru/ambil_matpel/" + id,
		success: function (data) {
			if (data.status == "ok") {
				var jml_data = Object.keys(data.data).length;
				var hate = '<div class="modal fade" id="m_siswa_matpel" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 id="myModalLabel">Setting Mata Pelajaran</h4></div><div class="modal-body"><form name="f_siswa_matpel" id="f_siswa_matpel" method="post" onsubmit="return m_guru_matpel_s();"><input type="hidden" name="id_mhs" id="id_mhs" value="' + id + '"><div id="konfirmasi"></div>';
...
function m_guru_matpel_s() {
	var f_asal = $("#f_siswa_matpel");
	var form = getFormData(f_asal);

	$.ajax({
		type: "POST",
		url: base_url + "adm/m_guru/simpan_matpel",
		data: JSON.stringify(form),
		dataType: 'json',
		contentType: 'application/json; charset=utf-8'
	}).done(function (response) {
...
$("#pilih_mapel_for_materi").change(function () {
	var id_mapel = this.value;
	window.location.assign(base_url + "adm/m_materi/pilih_mapel/" + id_mapel);
});
